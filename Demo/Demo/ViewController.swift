...
class ViewController: UIViewController {
    ...
    private lazy var horizontalScrollView: SlideshowView = {
        let sv = SlideshowView()
    ...
    private lazy var verticalScrollView: SlideshowView = {
        let sv = SlideshowView()
...
extension ViewController: SlideshowViewDataSource {
    ...
    func slideshowView(_ slideshowView: SlideshowView, cellForItemAt index: Int) -> UICollectionViewCell {
        if slideshowView === horizontalScrollView {
            let cell = slideshowView.dequeueReusableCell(withReuseIdentifier: horizontalCellId, at: index)
            ...
            let cell = slideshowView.dequeueReusableCell(withReuseIdentifier: verticalCellId, at: index)
...
extension ViewController: SlideshowViewDelegate {
    ...
    func slideshowView(_ slideshowView: SlideshowView, willDisplay cell: UICollectionViewCell, forItemAt index: Int) {
        let tag = slideshowView === horizontalScrollView ? 100 : 200
        guard let indicator = view.viewWithTag(tag) as? UIPageControl else { return }
        indicator.currentPage = index
    }
   
    func slideshowView(_ slideshowView: SlideshowView, didHighlightItemAt index: Int) {
        let direction = slideshowView === horizontalScrollView ? "Horizontal" : "Vertical"
